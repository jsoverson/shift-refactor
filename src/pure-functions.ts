import { FunctionDeclaration, Node, Script, CallExpression } from "shift-ast";
import shiftScope, { ScopeLookup, Variable, Reference } from "shift-scope";
import { traverse } from 'shift-traverser';
import { SimpleIdentifier } from ".";
import { isMemberAssignment, isMemberExpression, isDeepSimilar, getRootIdentifier } from "./util";
  ...
  constructor(fnNode: AssessmentNode, options: PureFunctionAssessmentOptions = {}) {
    ...
    traverse(tempProgram, {
      enter: function(node: Node, parent: Node) {
        ...
        } else if (isMemberExpression(node) && isMemberAssignment(parent) && node.object.type === 'IdentifierExpression') {
          const variable = lookup(node.object);
